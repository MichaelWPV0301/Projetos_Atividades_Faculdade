...
instrucoesComRegs = {
    "ADD": "0x800", "SHR": "0x900", "SHL": "0xa00", "NOT": "0xb00",
    "AND": "0xc00", "OR": "0xd00", "XOR": "0xe00", "CMP": "0xf00",
    "LD": "0x000", "ST": "0x100", "JMPR":"0x300", "DATA": "0x200"
}

# instrucoesSemRegs: instruções que não usam os registradores
instrucoesSemRegs = {
    "JMP": "0x4000", "CLF": "0x6000", "J": "0x500"
}

# outside: instruções de periféricos, convertidas para bits
outside = {"BASE": "0x700", "IN": "0", "OUT": "1", "DATA": "0", "ADDR": "1"}
...
def converte_hexadecimal(valor, reg=0):
    """
...
def verifica_label(partes):
    ...
    if ":" in partes[0]:
        label[partes[0]] = f"0x00{endereco+1:02x}"
        partes = partes[1:]
...
def converte_registradores(ra, rb):
    ...
    hexaregs = converte_hexadecimal(f"0b{regs}", 1)
        ...
        if partes:  # Se houver instrução na linha
            ...
            # INSTRUÇÕES ESPECIAIS (NÃO ESTÃO NOS SETS)/
                ...
                # HALT: simula parada como JMP para próxima linha
                    ...
                    adiciona_instrucao(hexainstru)
                    if(verifica_intervalo(f"0x{endereco:04x}")):
                        adiciona_instrucao(f"0x{endereco:04x}")
                    else:
