...
PROCESS_DIVIDEND:
    ADD R3, R1       ; R1 = R3 + R1. Converte o dígito da dezena (R1) de ASCII para seu valor numérico. (Resultado em R1)
...
LOOP_MULTIPLY_TEN:
    ...
    DATA R0, 0x00    ; R0 carrega 0 para a comparação.
...
END_DIVIDEND_CALC:
    MOVE R2, R0      ; R0 = R2. Move o valor final do dividendo (de R2) para R0. (Destino R0)
    JMP PREPARE_DIVISOR ; Prossegue para preparar a entrada do divisor.

; --- Manipulação de Dividendo de Um Dígito ---
HANDLE_SINGLE_DIGIT:
    MOVE R1, R0      ; R0 = R1. Move o dígito único (de R1) para R0. (Destino R0)
    JMP PREPARE_DIVISOR ; Prossegue para preparar a entrada do divisor.

; --- Preparar e Ler Divisor ---
PREPARE_DIVISOR:
    DATA R3, 0x30    ; R3 carrega ASCII '0'.
...
LOOP_READ_DIVISOR:
    IN DATA, R1      ; Lê o caractere do divisor para R1.
...
; --- Manipulador de Erro ---
INVALID_INPUT:
    DATA R0, 0x21    ; R0 = '!' (caractere de erro).
...
START_DIVISION:
    DATA R3, 0x00    ; R3 = 0 (este registrador armazenará o quociente).
...
LOOP_DIVISION:
    CMP R2, R0       ; Compara dividendo (R0) com divisor (R2).
...
DISPLAY_RESULT:
    MOVE R3, R0      ; R0 = R3. Move o quociente (de R3) para R0. (Destino R0)
...
LOOP_DISPLAY:
    CMP R0, R1       ; Compara o quociente (R0) com 9 (R1).
