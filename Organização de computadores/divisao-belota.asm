...
PREPARAR_VALOR: ADD R3, R1 ; Subtrai 0x30 (R3 = -0x30) de R1
...
CICLO_DEZENA_ACUMULACAO: DATA R0, 0x00 ; Zera R0 para a condição de parada do ciclo
...
VALOR_SIMPLES: MOVE R1, R2   ; Move o valor do segundo componente (R2) para R1 (destino final)
...
AVANCAR_PARA_SEGUNDO_VALOR: MOVE R2, R0   ; Move o valor acumulado (dezena * 10) para R2
    DATA R3, 0x00 ; Zera R3 para uso posterior

    ; Recarrega o delimitador em R2 para a próxima comparação de entrada
    DATA R2, 0x2F

CICLO_LEITURA_DIVISOR: IN DATA, R1   ; Obtém o caractere do divisor
...
ERRO_DE_SINAL: DATA R0, 0x21 ; Caractere '!' para indicar erro
...
INICIAR_OPERACAO: DATA R2, 0x00 ; Inicializa o registrador para o quociente (começa em zero)
...
CICLO_DE_DIVISAO: CMP R1, R0    ; Compara o dividendo (R1) com o divisor (R0)
...
APRESENTAR_RESULTADO: MOVE R2, R0   ; Move o quociente (que deveria estar em R0) para R2
...
CICLO_EXIBICAO: CMP R0, R2    ; Compara o quociente (R0) com o limite de dezena (9 em R2)
...
PROCESSAR_DEZENAS: ADD R3, R0    ; Reduz o quociente (R0) em 10 (R3 é -10)
...
FINALIZAR_EXECUCAO: HALT ; Interrompe o processador